...
/*const nameSearch = (param) => {
  param.forEach((name, index) => {
    return(`El estudiante ${name} se encuentra en la posición ${index}`);
  })
}
let newNameSearh = nameSearch(nameFinder);
console.log(newNameSearh);

//xk me da UNDEFINE!!!*/ 

const nameSearch = (param) => {
  param.forEach((name, index) => {
    console.log(`El estudiante ${name} se encuentra en la posición ${index}`);
  })
}
nameSearch(nameFinder);
